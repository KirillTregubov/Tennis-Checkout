...
import { itemsAtom, deleteItemAtom } from '@components/stores/Store'
...
export default function Checkout() {
    ...
    const staticHtml = (props) => {
        return (
            <div className="pb-6">
                ...
                <div className="flex items-center relative text-green-200 h-36">
	...
	if (!data) return (
        staticHtml(
            [1,2,3].map((product) => (
                <div key={product} className="px-8 h-32 sm:h-auto">
                     <ContentLoader 
                        uniqueKey={product}
                        speed={2}
                        width={700}
                        height={250}
                        viewBox="0 0 700 200"
                        backgroundColor={fullConfig.theme.textColor.green[200]}
                        foregroundColor={fullConfig.theme.textColor.green[500]}
                        style={{ width: '100%' }}>
                        <rect x="0" y="20" rx="16" ry="16" width="150" height="180" />
                        <rect x="170" y="28" rx="3" ry="3" width="150" height="20" />
                        <rect x="170" y="58" rx="3" ry="3" width="100" height="7" />

                        <rect x="430" y="28" rx="3" ry="3" width="50" height="20" />
                        <rect x="430" y="58" rx="3" ry="3" width="50" height="7" />
                        <rect x="620" y="28" rx="3" ry="3" width="80" height="20" />
                    </ContentLoader>
                </div>
            ))
        )
                ...
                <div>
                    <div className="max-w-screen-xl mx-auto lg:px-12">
                        { items.length > 0 && items.map((item) => (
                            <div key={'cart-'+item.id} className="flex flex-col sm:flex-row px-8 py-4">
                                <div className="flex-shrink-0 relative h-56 w-full sm:w-56 bg-cover overflow-hidden rounded-md">
                                    <Image src={data.filter((product) => product.id === item.id)[0].image} alt={'Image of ' + data.filter((product) => product.id === item.id)[0].title} layout="fill" objectFit="cover" loading="eager" placeholder="blur" blurDataURL={`data:image/svg+xml;base64,${toBase64(shimmer(700, 475))}`} />
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-3 w-full sm:px-8 pt-4 sm:py-4 sm:pr-0">
                                    <div className="flex flex-col justify-between">
                                        <div>
                                            <h1 className="font-bold text-lg">{data.filter((product) => product.id === item.id)[0].title}</h1>
                                            <h2 className="text-gray-600">{data.filter((product) => product.id === item.id)[0].description}</h2>
                                        </div>
                                    ...
                                    <div className="flex flex-col sm:items-center order-last sm:order-none">
                                        <div>
                                            <NumberInput item={item} />
                                            <button className="font-semibold text-green-800 hover:text-green-600 my-2" onClick={() => deleteItem(item.id)}>Remove</button>
                                        </div>
                                    </div>
                                    <div className="text-2xl sm:text-right my-5 sm:my-0">
                                        <h1 className="font-bold">${data.filter((product) => product.id === item.id)[0].price}</h1>
                                    </div>
                                </div>
                            </div>
                        ))}
